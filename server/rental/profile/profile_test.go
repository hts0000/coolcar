...
func TestProfileLifecyle(t *testing.T) {
	...
	for _, cc := range cases {
		...
		if p.IdentityStatus != cc.wantStatus {
			t.Errorf("%s: status field incorrect: want %q, got %q", cc.name, cc.wantStatus, p.IdentityStatus)
		}
	}
}
...
func TestProfilePhotoLifecycle(t *testing.T) {
	...
	for _, cc := range cases {
		...
		if got != cc.wantURL {
			t.Errorf("%s: want %q, got %q", cc.name, cc.wantURL, got)
		}
	}
}
